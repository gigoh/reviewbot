 ...
 */
function formatSummaryComment(review: ReviewResult, config: Config): string {
  ...
  if (review.comments.length > 0) {
    comment += `---\n\n`;
    comment += `📝 **${review.comments.length} detailed comment(s)** have been posted on specific lines in the changed files.\n\n`;
  }

  comment += '---\n';
  comment += `_${getReviewMetadata(config)}_\n`;
